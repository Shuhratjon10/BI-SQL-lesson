...
-- i use the following tables to anser the question below:
...
Select Category, sum(StockQuantity) as totalnumber from Products group by Category;

--2.Using Products table, get the average price of products in the 'Electronics' category.

Select Category, avg(Price) as avarage from Products where Category='Electronics' group by Category ;
...
Select ProductName, Price as highestPrice from Products
where Price = (Select max(Price)from Products);

--7.Using Products table, label stock as 'Low Stock' if quantity < 30, else 'Sufficient'.

Select * from Products;
Select ProductName, StockQuantity, case when StockQuantity < 30 then 'Low Stock' else 'Sufficient' end as labelStock from Products;

--8.Using Customers table, find the total number of customers in each country.

Select * from Customers;
Select Country, count(*) as Totalnumber from Customers group by Country;
...
Select p.ProductID, p.ProductName AS OriginalName, pd.ProductName AS DiscountedName from Products p
join Products_Discounted pd ON p.ProductID = pd.ProductID;
...
Select * from Orders;
Select Year(OrderDate) as OrderYear, avg(TotalAmount) as Avarage from Orders group by Year(OrderDate);
; 
...
Select district_name,
	...
	Sum(Case when year='2013' then population else 0 end) as [2012]
...
Select year,
	...
	Sum(Case when district_name='Yashnobod' then Population else 0 end ) as Yakkasaroy,
	Sum(Case when district_name='Bektemir' then Population else 0 end) as Bektemir
from City_Population group by year;

--19.(Bektemir, Chilonzor, Yakkasaroy) and copy results to a new Population_Each_City table.
...
--                               Hard-Level Tasks
--20.Using Invoices table, show top 3 customers with the highest total invoice amount. Return CustomerID and Totalspent.

Select * from Invoices;
Select Top 3 CustomerID, sum(TotalAmount) as Totalspend  from Invoices group by CustomerID Order by Totalspend DESC;

--21.Transform Population_Each_Year table to its original format (City_Population).
Select * from Population_Each_Year;
union all
Select * from City_Population;
--22.Using Products and Sales tables, list product names and the number of times each has been sold. (Research for Joins)

Select * from Products, Sales;
Select p.ProductName,  Count (s.SaleID) AS NumberOfSales from Products p
LEFT JOIN Sales s on p.ProductID = s.ProductID
group by p.ProductName;
--23.Transform Population_Each_City table to its original format (City_Population).
