...
SELECT ProductName, Price as MinPrice FROM Products
...
Select FirstName, LastName, DepartmentName, Salary as MaxSAlary from Employees 
...
Select * from Sales where SaleID = 7

Select Sum(SaleAmount) as TotalSales from Sales Where  SaleID = 7
...
SELECT FirstName, LastName, DepartmentName, Age as AvarageAge FROM Employees
...
Select DepartmentName, COUNT(*) as NumEmployees from Employees group by DepartmentName;
...
Select C.CustomerID, C.FirstName, SUM(S.SaleAmount) as TotalSales from Sales S
...
Select DepartmentName, count(*) as amountEmployee from Employees  group by DepartmentName
having  count(*)>5 ;
...
Select ProductID, sum(SaleAmount) as TotalSale, avg(SaleAmount) as AvarageSale from Sales group by ProductID;

--12.Write a query to count the number of employees from the Department HR.

Select * from Employees;
Select count(*) as 'Count HR employee' from Employees Where DepartmentName='HR';
...
Select * from Employees;
Select DepartmentName, max(Salary) as highestSalary, min(Salary) as lowestSalary  from Employees group by DepartmentName;

--14.Write a query to calculate the average salary per Department.(DeptID is enough, if you don't have DeptName).

Select * from Employees;

Select DepartmentName, avg(Salary) as avarageSalary from Employees group by DepartmentName;
...
Select DepartmentName, avg(Salary) as avarageSalary, count(*) as TotalEmployee from Employees group by DepartmentName;
...
Select SaleDate, sum(SaleAmount) as TotalSale from Sales group by SaleDate;

--18.Write a query to show the list of customers who placed at least 3 orders.

Select * from Orders;
Select CustomerId, sum(Quantity) as totalOrder from Orders group by CustomerID having sum(Quantity)>3;
...
Select DepartmentName, avg(Salary) as greaterSalary from Employees group by DepartmentName having avg(Salary)>60000;
...
Select * from Products;
Select Category, avg(price) as avaragePrice from Products group by Category having avg(Price)>150;
...
select * from Employees;
select DepartmentName, sum(Salary) as Total from Employees group by DepartmentName;

select DepartmentName, sum(Salary) as TotalSalary, avg(Salary) as AvarageSalary from Employees group by DepartmentName having avg(Salary)>65000;
...
Select OrderID, TotalAmount from Orders where TotalAmount>50  group by OrderID, TotalAmount;
...
Select * from Orders;
Select ProductID, OrderDate, sum(TotalAmount) as TottalSale, count(*) as uniqProduct from Orders group by ProductID, OrderDate;

--25.Write a query to find the MIN and MAX order quantity per Year. From orders table. Necessary tables:

Select * from Orders;

Select OrderDate, min(quantity) as 'min order' , max (quantity) as 'max order' from Orders group by OrderDate; 
